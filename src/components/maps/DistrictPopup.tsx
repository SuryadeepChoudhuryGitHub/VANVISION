...
export const DistrictPopup: React.FC<DistrictPopupProps> = ({ district, onInspect }) => {
  return (
    <div className="w-72 p-4 bg-white text-slate-800 rounded-xl">
      {/* Header */}
      <div className="flex items-start justify-between border-b border-slate-100 pb-2.5 mb-2.5 pr-6">
        <div>
          <h4 className="font-bold text-slate-900 text-sm leading-tight">{district.name}</h4>
          <span className="text-[11px] text-slate-500 font-medium">{district.state}</span>
        </div>
      ...
      <div className="mb-3 p-2.5 bg-amber-50/80 border border-amber-200/90 rounded-lg text-[11px]">
        <div className="flex justify-between items-center text-amber-950 font-bold mb-1">
          <span>Cadastral Hotspot Issue</span>
          <span className="font-mono text-xs text-amber-900">{district.riskScore}/100</span>
        </div>
        <p className="text-amber-900/90 leading-relaxed text-[11px]">{district.primaryIssue}</p>
      </div>
      ...
      {onInspect && (
        <button
          ...
          className="w-full py-2 px-3 bg-forest-850 hover:bg-forest-900 text-white rounded-lg font-semibold text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-xs"
        >
